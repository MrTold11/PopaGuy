...
func BotBuild() {
	fmt.Println("TESTING")
	counter := 0
	data, er := os.ReadFile("internal/botbuild/botTokenTelegram.txt")
	if er != nil {
		fmt.Println("bad")
		return
	}
	...
	data, er = os.ReadFile("internal/botbuild/tradingToken.txt")
	if er != nil {
		fmt.Println("bad")
		return
	}
						...
						if Err != nil {
							...
							if err != nil {
								...
								if resp.StatusCode == 200 {
									...
									fmt.Println(operId)
									persCode = false
									amount, err := os.ReadFile("internal/botbuild/numberInMap.txt")
									if err != nil {
										fmt.Println("OP")
										return
									}
									...
									_ = os.WriteFile("internal/botbuild/numberInMap.txt", []byte (stree), 0644)


									m := make(map[int64] string)
									data, err := os.ReadFile("internal/botbuild/reklMap.txt")
									if err != nil {
										fmt.Println(err)
									}
									...
									err = os.WriteFile("internal/botbuild/reklMap.txt", REQ, 0644)

									data, err = os.ReadFile("internal/botbuild/reklTimeMap.txt")
									ma := make(map[int64] int64)
									err = json.Unmarshal(data, &ma)
									ma[int64(am)] = int64(dayAM)
									REQ, _ = json.Marshal(ma)
									err = os.WriteFile("internal/botbuild/reklTimeMap.txt", REQ, 0644)
